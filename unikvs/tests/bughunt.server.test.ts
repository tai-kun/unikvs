...
describe("バグ調査: Transformer", () => {
  ...
  // BUG-001: 配列形式の context が record スキーマにより {"0": [...]} 形式のオブジェクトに
  // 変換され、チェックサム検証がサイレントにスキップされる問題。
  test("チェックサム不一致 (配列形式 context) でもストリーム書き込みが拒否される", async () => {
    const mem = new Memory();
  ...
  test("配列形式 context はオブジェクト形式と同等に扱われる", async () => {
    ...
    // ChecksumRequiredError が誤発生しないこと (required チェックが正しく満たされること) の確認も兼ねる
    await kvs.close();
  });
