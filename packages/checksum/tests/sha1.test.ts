...
test("CHECKSUM_VAR_NAME が定義されている", ({ expect }) => {
  // 検証
  expect(ChecksumSha1.CHECKSUM_VAR_NAME).toBe("@unikvs/checksum:sha1");
});
...
test("期待値と一致するチェックサムを指定して encode すると、SHA-1 のハッシュで検証してデータをそのまま返す", ({
  ...
  // 準備
  const checksum = new ChecksumSha1();
  const vars = { [ChecksumSha1.CHECKSUM_VAR_NAME]: VALID_HASH };
...
test("空のデータを SHA-1 のハッシュ値で検証できる", ({ expect }) => {
  ...
  const vars = { [ChecksumSha1.CHECKSUM_VAR_NAME]: EMPTY_HASH };
...
test("期待値と一致しないチェックサムを指定すると、実際のハッシュ値を含む ChecksumMismatchError を投げる", ({
  ...
  // 準備
  const checksum = new ChecksumSha1();
  const vars = { [ChecksumSha1.CHECKSUM_VAR_NAME]: INVALID_HASH };
