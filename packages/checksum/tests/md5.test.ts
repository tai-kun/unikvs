...
test("CHECKSUM_VAR_NAME が定義されている", ({ expect }) => {
  // 検証
  expect(ChecksumMd5.CHECKSUM_VAR_NAME).toBe("@unikvs/checksum:md5");
});
...
test("期待値と一致するチェックサムを指定して encode すると、MD5 のハッシュで検証してデータをそのまま返す", ({
  ...
  // 準備
  const checksum = new ChecksumMd5();
  const vars = { [ChecksumMd5.CHECKSUM_VAR_NAME]: VALID_HASH };
...
test("空のデータを MD5 のハッシュ値で検証できる", ({ expect }) => {
  ...
  const vars = { [ChecksumMd5.CHECKSUM_VAR_NAME]: EMPTY_HASH };
...
test("期待値と一致しないチェックサムを指定すると、実際のハッシュ値を含む ChecksumMismatchError を投げる", ({
  ...
  // 準備
  const checksum = new ChecksumMd5();
  const vars = { [ChecksumMd5.CHECKSUM_VAR_NAME]: INVALID_HASH };
